...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const sessionId = "TEFAGHO-MD~" + base64;

                    await sock.sendMessage(sock.user.id, { text: sessionId });

                    const successMsg = {
                        text:
                            `🚀 *TEFAGHO-MD Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on GitHub\n\n` +
                            `_Powered by TEFAGHO-MD\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ OFC SITE: https://project-explores.onrender.com\n` +
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: "120363403385036650@newsletter",
                                newsletterName: "𝗧𝗘𝗙𝗔𝗚𝗛𝗢-𝗠𝗗",
                                serverMessageId: 143
                            }
                        }
                    };
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "TIEGOTEE");
            if (!res.headersSent) {
